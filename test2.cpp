...
struct Thief2<T, field, Idx> {
  friend const auto& steal_impl(T& t, Idx_t<Idx>) { return (t.*field)(); }
};

template <typename T, auto (T::*field)(), std::size_t Idx>
struct Thief2<T, field, Idx> {
  friend auto& steal_impl(T& t, Idx_t<Idx>) { return (t.*field)(); }
};
...
#define STRUCT_PACK_VISIT_PRIVATE_HELPER(Idx, X, Bank)                       \
  ...
  struct is_private_member_field<T, Idx, std::void_t<decltype(T{}.X)> >      \
      : std::false_type {};                                                  \
  template struct ThiefMemberFunction<                                       \
      Bank, member_function_test<Bank>(&Bank::X), Idx>;                      \
  template struct ThiefMemberFunctionConst<                                  \
      Bank, const_member_function_test<Bank>(&Bank::X), Idx>;
