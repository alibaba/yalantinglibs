...
using varint32_t = detail::varint<int32_t>;
using varuint32_t = detail::varint<uint32_t>;
using varint64_t = detail::varint<int64_t>;
using varuint64_t = detail::varint<uint64_t>;

using sint32_t = detail::sint<int32_t>;
using sint64_t = detail::sint<int64_t>;
...
consteval auto get_field_varint_type() {
  ...
  else if constexpr (detail::varintable_t<T> || detail::sintable_t<T>) {
...
consteval std::size_t calculate_tag_size() {
  ...
  return detail::calculate_varint_size(tag);
}
  ...
  if constexpr (detail::variant<Field>) {
    ...
    if constexpr (detail::optional<Field>) {
      ...
      if constexpr (std::is_enum_v<Field>) {
        ...
        return tag_size + detail::calculate_varint_size(v);
      }
      else {
        if (field == Field{}) {
          return 0;
        }
        return tag_size + detail::calculate_varint_size(field);
      }
    }
      ...
      if constexpr (std::same_as<Field, std::string>) {
        ...
        return tag_size + detail::calculate_varint_size(field.size()) +
               field.size();
      }
        ...
        if constexpr (VARINT<value_type>) {
          ...
          return tag_size + detail::calculate_varint_size(field.size()) + sz;
        }
        else if constexpr (I32<value_type> || I64<value_type>) {
          auto total = field.size() * sizeof(value_type);
          return tag_size + detail::calculate_varint_size(total) + total;
        }
        else {
          ...
          for (auto&& e : field) {
            ...
            total += tag_size + detail::calculate_varint_size(size) + size;
          }
          return total;
        }
      }
        ...
        return tag_size + detail::calculate_varint_size(size) + size;
      }
      else {
...
class packer {
 ...
 private:
    ...
    if constexpr (detail::optional<Field>) {
      ...
      if constexpr (VARINT<Field>) {
        if constexpr (std::is_enum_v<Field>) {
          ...
          // why '0'
          data_[pos_++] = '0';
          detail::serialize_varint(data_, pos_, v);
        }
        else {
          if (field == Field{}) {
            return;
          ...
          detail::serialize_varint(data_, pos_, field);
        }
      }
        ...
        if constexpr (std::same_as<Field, std::string>) {
          ...
          write_tag<FieldNumber, wire_type>();
          detail::serialize_varint(data_, pos_, field.size());
          ...
          if constexpr (VARINT<value_type>) {
            ...
            std::size_t sz = 0;
            for (auto&& e : field) {
              sz += detail::calculate_varint_size(e);
              detail::serialize_varint(data_, pos_, e);
            }
            pos_ = sz_pos;
            auto new_pos = pos_;
            detail::serialize_varint(data_, pos_, sz);
            pos_ = new_pos;
          }
          else if constexpr (I64<value_type> || I32<value_type>) {
            write_tag<FieldNumber, wire_type>();
            auto size = field.size() * sizeof(value_type);
            detail::serialize_varint(data_, pos_, size);
  ...
  STRUCT_PACK_INLINE void write_tag() {
    ...
    detail::serialize_varint(data_, pos_, tag);
  }
 ...
 private:
  ...
  [[nodiscard]] STRUCT_PACK_INLINE std::errc deserialize_one_impl(T& t,
    ...
    if constexpr (WireType == wire_type_t::varint) {
      ...
      if constexpr (detail::optional<field_type>) {
        f = typename field_type::value_type{};
        return deserialize_one_impl<T, WireType>(t, f.value());
      }
      else {
        return detail::deserialize_varint(data_, pos_, size_, f);
      }
    }
  ...
  [[nodiscard]] STRUCT_PACK_INLINE std::errc deserialize_len(T& t, Field& f) {
    if constexpr (detail::optional<Field>) {
      ...
      uint64_t sz = 0;
      auto ec = detail::deserialize_varint(data_, pos_, size_, sz);
      if (ec != std::errc{}) [[unlikely]] {
        return ec;
      }
      f.resize(sz);
      ...
      if constexpr (VARINT<value_type>) {
        ...
        while (pos_ < max_pos) [[likely]] {
            uint64_t val = 0;
            ec = detail::deserialize_varint(data_, pos_, size_, val);
            if (ec != std::errc{}) [[unlikely]] {
              return ec;
            }
            f.push_back(val);
  ...
  [[nodiscard]] STRUCT_PACK_INLINE std::errc read_tag(T& t, uint32_t& tag) {
    return detail::deserialize_varint(data_, pos_, size_, tag);
  }
