...
namespace neon {
    ...
    inline std::vector<StringLike> simd_str_split(std::string_view string, const char delim) {
        ...
        for (size_t i = 0; i < aligned32_size; i += 32) {
            ...
            uint8x16_t data2 = vld1q_u8(reinterpret_cast<const uint8_t*>(&pstr[i + 16]));
            uint8x16_t result1 = vceqq_u8(data1, match);
            uint8x16_t result2 = vceqq_u8(data2, match);
            // Quickly fold the 256-bit detection results to 64-bit. It cannot be accurately located, but can be used for quick skipping.
            ...
            if (result64[0] != 0) {
                ...
                // The final fold will form 32 hit marks on 64 bits, and the alternate bits will take effect.
                // For example, if all hits are found, the result will be 0x55555555555555555UL
                uint8x16_t vmask = vreinterpretq_u8_u64(vdupq_n_u64(0x4010040140100401UL));
        ...
        if (aligned32_size < aligned16_size) {
            uint8x16_t data = vld1q_u8(reinterpret_cast<const uint8_t*>(&pstr[aligned32_size]));
