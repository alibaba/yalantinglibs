...
class AbstractSmoothRateLimiter : public RateLimiter {
 protected:
  virtual void doSetRate(double permits_per_second, double stable_internal_micros) = 0;
  virtual long storedPermitsToWaitTime(double stored_permits, double permits_to_take) = 0;
  virtual double coolDownInternalMicros() = 0;
  void resync(long now_micros) {
    // if next_free_ticket is in the past, resync to now
    ELOG_DEBUG << "now micros: " << now_micros << ", next_free_ticket_micros_: " << this->next_free_ticket_micros_;
    if (now_micros > this->next_free_ticket_micros_) {
      double newPermits = (now_micros - this->next_free_ticket_micros_) / coolDownInternalMicros();
      this->stored_permits_ = std::min(this->max_permits_, this->stored_permits_ + newPermits);
  ...
  long reserveEarliestAvailable(int required_permits, long now_micros) {
    ...
    double stored_permits_to_spend = std::min((double) required_permits, this->stored_permits_);
    double fresh_permits = required_permits - stored_permits_to_spend;
    long wait_micros = storedPermitsToWaitTime(this->stored_permits_, stored_permits_to_spend) 
   ...
   * per second has a stable internal of 200ms.
   */
...
class SmoothBurstyRateLimiter : public AbstractSmoothRateLimiter {
 ...
 protected:
  void doSetRate(double permits_per_second, double stable_internal_micros) {
    ...
    this->stored_permits_ = (0 == old_max_permits) ? 0 
      : this->stored_permits_ * this->max_permits_ / old_max_permits;
    ELOG_DEBUG << "max_permits_: " << this->max_permits_ << ", stored_permits_:" << this->stored_permits_;
  }
  ...
  double coolDownInternalMicros() {
    return this->stable_internal_micros_;
  }

 private:
  /** 
   * The work(permits) of how many seconds can be saved up if the RateLimiter is unused.
   */
