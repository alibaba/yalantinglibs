...
class ip_whitelist {
 public:
  ...
  bool add_ip_range(const std::string& start_ip, const std::string& end_ip) {
    ...
    try {
      ...
      if (start_addr.is_v4() != end_addr.is_v4()) {
        ELOG_ERROR << "IP version mismatch in range: " << start_ip << " - "
                   << end_ip;
        return false;
      }

      ip_ranges_.emplace_back(start_addr, end_addr);
      ELOG_INFO << "Added IP range to whitelist: " << start_ip << " - "
                << end_ip;
      return true;
    } catch (const std::exception& e) {
  ...
  /**
   * @brief Universal add method - automatically detects format and adds to
   * whitelist
   * @param entry String entry that can be: single IP, CIDR network, IP range,
   * or regex pattern
   * @return true if successfully added, false if format error
   *
   * Format detection:
   ...
   * - Contains regex special chars -> Regex pattern (e.g.,
   * "192\.168\.1\.[0-9]+")
  ...
  size_t size() const {
    ...
    return single_ips_.size() + cidr_v4_networks_.size() +
           cidr_v6_networks_.size() + ip_ranges_.size() +
           regex_patterns_.size();
  }
  ...
  bool empty() const {
    ...
    return single_ips_.empty() && cidr_v4_networks_.empty() &&
           cidr_v6_networks_.empty() && ip_ranges_.empty() &&
           regex_patterns_.empty();
  }
  ...
  std::vector<std::string> get_ip_ranges() const {
    ...
    for (const auto& range : ip_ranges_) {
      result.push_back(range.first.to_string() + " - " +
                       range.second.to_string());
    }
