...
public:
 ...
 public:
  ...
  async_simple::coro::Lazy<std::error_code> accept(asio::ip::tcp::socket& soc) noexcept {
    ...
    char buffer[sz.size()];
    auto [ec2, canceled] = co_await async_simple::coro::collectAll<async_simple::SignalType::Terminate>(async_read(soc,asio::buffer(buffer)),coro_io::sleep_for(std::chrono::milliseconds{100000000}));
