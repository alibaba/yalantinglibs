...
async_read(coro_io::ib_socket_t& ib_socket, std::span<char> buffer) noexcept {
  ...
  if (!ib_socket.get_config().enable_zero_copy) {
    do {
      ...
      auto [ec, len] = co_await ib_socket.async_io<ib_socket_t::read>(
          ib_buffer_view_t{ib_buffer[0]});
      if (ec) {
        co_return std::pair{ec, 0};
      }
      int buffer_index = 1;
      for (; total_size_read + len < buffer.size(); buffer_index ^= 1) {
        auto [result, result2] = co_await async_simple::coro::collectAll(
            ib_socket.async_io<ib_socket_t::read>(
                ib_buffer_view_t{ib_buffer[buffer_index]}),
            [address = ib_buffer[buffer_index]->addr, len = len,
             dst = buffer.data() +
                   total_size_read]() -> async_simple::coro::Lazy<void> {
      ...
      memcpy(buffer.data() + total_size_read, ib_buffer[buffer_index]->addr,
             len);
      total_size_read += len;
      co_return std::pair{std::error_code{}, total_size_read};
    } while (false);
  }
  ib_buffer[0] = ib_buffer_t::regist(ib_socket.get_device(), buffer.data(),
                                     std::min(buffer.size(), block_size));
  for (; total_size_read < buffer.size();) {
    std::span<char> next_buffer = {
        (char*)buffer.data() + total_size_read + ib_buffer[0]->length,
        std::min(buffer.size() - total_size_read - ib_buffer[0]->length,
                 block_size)};
    auto [ec, size] = co_await ib_socket.async_io<ib_socket_t::read>(
        ib_buffer[0], next_buffer);
    if (ec) [[unlikely]] {
...
inline async_simple::coro::Lazy<std::pair<std::error_code, std::size_t>>
async_read(coro_io::ib_socket_t& ib_socket, ib_buffer_view_t buffer) noexcept {
  std::size_t total_size_read = 0;
...
async_read_some(coro_io::ib_socket_t& ib_socket,
                ib_buffer_view_t buffer) noexcept {
...
inline async_simple::coro::Lazy<std::pair<std::error_code, std::size_t>>
async_write(coro_io::ib_socket_t& ib_socket, std::span<char> buffer) noexcept {
  std::size_t block_size =
      std::min<std::size_t>(ib_socket.get_remote_buffer_size(), buffer.size());
  ib_buffer_t ib_buffer[2];
  int buffer_index = 0;
  if (!ib_socket.get_config().enable_zero_copy) {
    do {
      ...
      for (; total_size_write < buffer.size();
           buffer_index ^= 1, total_size_write += block_size) {
        auto view = ib_buffer_view_t{ib_buffer[buffer_index]->lkey,
                                     ib_buffer[buffer_index]->addr, block_size};
        block_size = std::min<std::size_t>(ib_socket.get_remote_buffer_size(),
                                           buffer.size() - total_size_write);
        auto [result, result2] = co_await async_simple::coro::collectAll(
            ib_socket.async_io<ib_socket_t::write>(view),
            [address = ib_buffer[buffer_index]->addr, block_size,
             src = buffer.data() +
                   total_size_write]() -> async_simple::coro::Lazy<void> {
  ...
  ib_buffer[0] =
      ib_buffer_t::regist(ib_socket.get_device(), buffer.data(), block_size);
  for (; total_size_write < buffer.size();) {
    std::span<char> next_buffer = {
        (char*)buffer.data() + total_size_write,
        std::min(buffer.size() - total_size_write, block_size)};
    auto [ec, len] = co_await ib_socket.async_io<ib_socket_t::write>(
        ib_buffer[0], next_buffer);
    if (ec) [[unlikely]] {
...
inline async_simple::coro::Lazy<std::pair<std::error_code, std::size_t>>
async_write(coro_io::ib_socket_t& ib_socket, ib_buffer_view_t buffer) noexcept {
  std::size_t block_size = std::min<std::size_t>(
      ib_socket.get_remote_buffer_size(), buffer.length());
  std::size_t total_size_write = 0;
  for (; total_size_write < buffer.length();) {
    std::span<char> next_buffer = {
        ...
        std::min(buffer.length() - total_size_write, block_size)};
