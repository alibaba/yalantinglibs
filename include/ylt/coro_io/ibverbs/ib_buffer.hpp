...
class mr_buffer_pool_t : public std::enable_shared_from_this<mr_buffer_pool_t> {
 private:
  ...
  void enqueue(mr_buffer_t client) {
 ...
 public:
  ...
  mr_buffer_t get_buffer() {
    ...
    if (!buffer) {
      ...
      buffer =
          mr_buffer_t::regist(device_->pd(), ptr, pool_config_.buffer_size);
    }
