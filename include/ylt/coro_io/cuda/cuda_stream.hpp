...
namespace coro_io {
  ...
  class cuda_event_watcher: public std::enable_shared_from_this<cuda_event_watcher> {
    private:
    ...
    public:
      ...
      cuda_event_watcher(bool bind_cpu = false):io_context_pool_(1, bind_cpu),executor_(io_context_pool_.get_executor()),timer_(executor_) {          
      }
      static std::shared_ptr<cuda_event_watcher> get_instance(coro_io::ExecutorWrapper<>* executor = nullptr) {
        static auto instance = get_instance_impl(executor);
        return instance;
      }

    private:
      static std::shared_ptr<cuda_event_watcher> get_instance_impl(coro_io::ExecutorWrapper<>* executor = nullptr) {
      ...
      async_simple::coro::Lazy<void> watch_event() {
        ...
        while (true) {
          ...
          if (dur >= sleep_interval_) {
            // ELOG_TRACE << "start sleep! dur before last active work= " << dur/std::chrono::milliseconds{1} << "ms";
    ...
    public:
      ...
      static void post(async_simple::Promise<CUresult> && promise, cuda_event_t&& event) {
        auto self = get_instance();
        self->events_queue_.enqueue(event_node_t{std::move(promise), std::move(event)});
