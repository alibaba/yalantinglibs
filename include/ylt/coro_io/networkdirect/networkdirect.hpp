...
#define IOCTL_ND(r_, i_)                                              \
  CTL_CODE(FILE_DEVICE_NETWORK, ND_FUNCTION(r_, i_), METHOD_BUFFERED, \
           FILE_ANY_ACCESS)

#define ND_FUNCTION_FROM_CTL_CODE(ctrlCode_) ((ctrlCode_ >> 2) & 0xFFF)
#define ND_RESOURCE_FROM_CTL_CODE(ctrlCode_) \
  (ND_FUNCTION_FROM_CTL_CODE(ctrlCode_) >> 6)
#define ND_OPERATION_FROM_CTRL_CODE(ctrlCode_) \
  (ND_FUNCTION_FROM_CTL_CODE(ctrlCode_) & 0x3F)
...
__inline NTSTATUS NdValidateMemoryMapping(__in const ND_MAPPING* pMapping,
                                          ND_ACCESS_TYPE AccessType,
                                          ULONG CbLength) {
...
__inline NTSTATUS NdValidateCoallescedMapping(__in const ND_MAPPING* pMapping,
                                              ND_ACCESS_TYPE AccessType,
                                              ULONG CbLength) {
  if (pMapping->MapType != NdMapMemoryCoallesce &&
      pMapping->MapType != NdMapPagesCoallesce) {
    return STATUS_INVALID_PARAMETER;
  }

  if (pMapping->MapMemory.AccessType != AccessType ||
      BYTE_OFFSET(pMapping->MapMemory.Address) + pMapping->MapMemory.CbLength >
          PAGE_SIZE ||
      pMapping->MapMemory.CbLength != CbLength) {
    return STATUS_INVALID_PARAMETER;
  }
...
__inline NTSTATUS NdValidateIoSpaceMapping(__in const ND_MAPPING* pMapping,
                                           ND_CACHING_TYPE CacheType,
                                           ULONG CbLength) {
...
__inline void NdThunkAdapterInfo(__out ND2_ADAPTER_INFO32* pInfo32,
  ...
  pInfo32->MaxRegistrationSize =
      (ULONG)(min(ULONG_MAX, pInfo->MaxRegistrationSize));
...
inline constexpr IID IID_IND2Overlapped = {
    ...
    0xb016,
    0x4a40,
    {0xa6, 0xf7, 0x62, 0x27, 0x91, 0xa7, 0x4, 0x4c}};

DECLARE_INTERFACE_(IND2Overlapped, IUnknown) {
  // *** IUnknown methods ***
  IFACEMETHOD(QueryInterface)
  (THIS_ REFIID riid, __deref_out LPVOID * ppvObj) PURE;
  ...
  STDMETHOD(CancelOverlappedRequests)(THIS) PURE;

  STDMETHOD(GetOverlappedResult)
  (THIS_ __in OVERLAPPED * pOverlapped, BOOL wait) PURE;
};
...
inline constexpr IID IID_IND2CompletionQueue = {
    ...
    0x64a0,
    0x4cbb,
    {0xaa, 0x75, 0xf6, 0xa7, 0x25, 0x1f, 0xda, 0x9e}};
...
DECLARE_INTERFACE_(IND2CompletionQueue, IND2Overlapped) {
  ...
  // *** IND2Overlapped methods ***
  IFACEMETHOD(CancelOverlappedRequests)(THIS) PURE;
  ...
  (THIS_ __out USHORT * pGroup, __out KAFFINITY * pAffinity) PURE;

  STDMETHOD(Resize)(THIS_ ULONG queueDepth) PURE;

  STDMETHOD(Notify)(THIS_ ULONG type, __inout OVERLAPPED * pOverlapped) PURE;

  STDMETHOD_(ULONG, GetResults)
  (THIS_ __out_ecount_part(nResults, return) ND2_RESULT results[],
   ULONG nResults) PURE;
};
...
inline constexpr IID IID_IND2SharedReceiveQueue = {
    ...
    0x459a,
    0x4db1,
    {0x82, 0x6b, 0x56, 0xcf, 0xcc, 0x27, 0x88, 0x83}};
...
DECLARE_INTERFACE_(IND2SharedReceiveQueue, IND2Overlapped) {
  ...
  STDMETHOD(Modify)(THIS_ ULONG queueDepth, ULONG notifyThreshold) PURE;

  STDMETHOD(Notify)(THIS_ __inout OVERLAPPED * pOverlapped) PURE;

  STDMETHOD(Receive)
  (THIS_ __in VOID * requestContext, __in_ecount_opt(nSge) const ND2_SGE sge[],
   ULONG nSge) PURE;
};
...
DECLARE_INTERFACE_(IND2MemoryWindow, IUnknown) {
  ...
  STDMETHOD_(UINT32, GetRemoteToken)(THIS) PURE;
};
...
DECLARE_INTERFACE_(IND2MemoryRegion, IND2Overlapped) {
  ...
  //////////////////////////////////
  // flags - Combination of ND_MR_FLAG_ALLOW_XXX.  Note remote flags imply
  // local.
  STDMETHOD(Register)
  (THIS_ __in_bcount(cbBuffer) const VOID* pBuffer, SIZE_T cbBuffer,
   ULONG flags, __inout OVERLAPPED* pOverlapped) PURE;

  STDMETHOD(Deregister)(THIS_ __inout OVERLAPPED * pOverlapped) PURE;

  STDMETHOD_(UINT32, GetLocalToken)(THIS) PURE;
...
DECLARE_INTERFACE_(IND2QueuePair, IUnknown) {
  ...
  STDMETHOD(Send)
  (THIS_ __in_opt VOID * requestContext,
   __in_ecount_opt(nSge) const ND2_SGE sge[], ULONG nSge, ULONG flags) PURE;

  STDMETHOD(Receive)
  (THIS_ __in_opt VOID * requestContext,
   __in_ecount_opt(nSge) const ND2_SGE sge[], ULONG nSge) PURE;

  // RemoteToken available thorugh IND2Mw::GetRemoteToken.
  STDMETHOD(Bind)
  (THIS_ __in_opt VOID * requestContext, __in IUnknown * pMemoryRegion,
   __inout IUnknown * pMemoryWindow, __in_bcount(cbBuffer) const VOID* pBuffer,
   SIZE_T cbBuffer, ULONG flags) PURE;

  STDMETHOD(Invalidate)
  (THIS_ __in_opt VOID * requestContext, __in IUnknown * pMemoryWindow,
   ULONG flags) PURE;

  STDMETHOD(Read)
  (THIS_ __in_opt VOID * requestContext,
   __in_ecount_opt(nSge) const ND2_SGE sge[], ULONG nSge, UINT64 remoteAddress,
   UINT32 remoteToken, ULONG flags) PURE;
...
DECLARE_INTERFACE_(IND2Connector, IND2Overlapped) {
  ...
  STDMETHOD(Bind)
  (THIS_ __in_bcount(cbAddress) const struct sockaddr* pAddress,
   ULONG cbAddress) PURE;

  STDMETHOD(Connect)
  (THIS_ __in IUnknown * pQueuePair,
   __in_bcount(cbDestAddress) const struct sockaddr* pDestAddress,
   ULONG cbDestAddress, ULONG inboundReadLimit, ULONG outboundReadLimit,
   __in_bcount_opt(cbPrivateData) const VOID* pPrivateData, ULONG cbPrivateData,
   __inout OVERLAPPED* pOverlapped) PURE;

  STDMETHOD(CompleteConnect)(THIS_ __inout OVERLAPPED * pOverlapped) PURE;

  STDMETHOD(Accept)
  (THIS_ __in IUnknown * pQueuePair, ULONG inboundReadLimit,
   ULONG outboundReadLimit,
   __in_bcount_opt(cbPrivateData) const VOID* pPrivateData, ULONG cbPrivateData,
   __inout OVERLAPPED* pOverlapped) PURE;

  STDMETHOD(Reject)
  (THIS_ __in_bcount_opt(cbPrivateData) const VOID* pPrivateData,
   ULONG cbPrivateData) PURE;

  STDMETHOD(GetReadLimits)
  (THIS_ __out_opt ULONG * pInboundReadLimit,
   __out_opt ULONG * pOutboundReadLimit) PURE;

  STDMETHOD(GetPrivateData)
  (THIS_ __out_bcount_opt(*pcbPrivateData) VOID * pPrivateData,
   __inout ULONG * pcbPrivateData) PURE;

  STDMETHOD(GetLocalAddress)
  (THIS_ __out_bcount_part_opt(*pcbAddress, *pcbAddress) struct sockaddr *
       pAddress,
   __inout ULONG * pcbAddress) PURE;
  ...
  STDMETHOD(NotifyDisconnect)(THIS_ __inout OVERLAPPED * pOverlapped) PURE;

  STDMETHOD(Disconnect)(THIS_ __inout OVERLAPPED * pOverlapped) PURE;
};
...
inline constexpr IID IID_IND2Listener = {
    ...
    0x3a57,
    0x4e02,
    {0x86, 0xa4, 0x35, 0x1, 0x65, 0xc2, 0xd1, 0x30}};
...
DECLARE_INTERFACE_(IND2Listener, IND2Overlapped) {
  ...
  STDMETHOD(Listen)(THIS_ ULONG backlog) PURE;
  ...
  (THIS_ __inout IUnknown * pConnector, __inout OVERLAPPED * pOverlapped) PURE;
};
...
inline constexpr IID IID_IND2Adapter = {
    ...
    0x4823,
    0x4d69,
    {0x84, 0x6c, 0xdf, 0xdc, 0xcf, 0xf9, 0xe5, 0xf3}};
...
DECLARE_INTERFACE_(IND2Adapter, IUnknown) {
  ...
  (THIS_ __deref_out HANDLE * phOverlappedFile) PURE;

  STDMETHOD(Query)
  (THIS_ __inout_bcount_opt(*pcbInfo) ND2_ADAPTER_INFO * pInfo,
   __inout ULONG * pcbInfo) PURE;

  STDMETHOD(QueryAddressList)
  (THIS_ __out_bcount_part_opt(*pcbAddressList, *pcbAddressList)
           SOCKET_ADDRESS_LIST *
       pAddressList,
   __inout ULONG * pcbAddressList) PURE;

  STDMETHOD(CreateCompletionQueue)
  (THIS_ __in REFIID iid, __in HANDLE hOverlappedFile, ULONG queueDepth,
   USHORT group, KAFFINITY affinity, __deref_out VOID * *ppCompletionQueue)
      PURE;

  STDMETHOD(CreateMemoryRegion)
  (THIS_ __in REFIID iid, __in HANDLE hOverlappedFile,
   __deref_out VOID * *ppMemoryRegion) PURE;

  STDMETHOD(CreateMemoryWindow)
  (THIS_ __in REFIID iid, __deref_out VOID * *ppMemoryWindow) PURE;

  STDMETHOD(CreateSharedReceiveQueue)
  (THIS_ __in REFIID iid, __in HANDLE hOverlappedFile, ULONG queueDepth,
   ULONG maxRequestSge, ULONG notifyThreshold, USHORT group, KAFFINITY affinity,
   __deref_out VOID * *ppSharedReceiveQueue) PURE;

  STDMETHOD(CreateQueuePair)
  (THIS_ __in REFIID iid, __in IUnknown * pReceiveCompletionQueue,
   __in IUnknown * pInitiatorCompletionQueue, __in_opt VOID * context,
   ULONG receiveQueueDepth, ULONG initiatorQueueDepth,
   ULONG maxReceiveRequestSge, ULONG maxInitiatorRequestSge,
   ULONG inlineDataSize, __deref_out VOID * *ppQueuePair) PURE;

  STDMETHOD(CreateQueuePairWithSrq)
  (THIS_ __in REFIID iid, __in IUnknown * pReceiveCompletionQueue,
   __in IUnknown * pInitiatorCompletionQueue,
   __in IUnknown * pSharedReceiveQueue, __in_opt VOID * context,
   ULONG initiatorQueueDepth, ULONG maxInitiatorRequestSge,
   ULONG inlineDataSize, __deref_out VOID * *ppQueuePair) PURE;

  STDMETHOD(CreateConnector)
  (THIS_ __in REFIID iid, __in HANDLE hOverlappedFile,
   __deref_out VOID * *ppConnector) PURE;

  STDMETHOD(CreateListener)
  (THIS_ __in REFIID iid, __in HANDLE hOverlappedFile,
   __deref_out VOID * *ppListener) PURE;
};
...
inline constexpr IID IID_IND2Provider = {
    ...
    0x76c4,
    0x46d0,
    {0x80, 0x3, 0x5c, 0x2e, 0xaa, 0x2c, 0x9a, 0x8e}};
...
DECLARE_INTERFACE_(IND2Provider, IUnknown) {
  ...
  (THIS_ __in_bcount(cbAddress) const struct sockaddr* pAddress,
   ULONG cbAddress, __out UINT64* pAdapterId) PURE;

  STDMETHOD(OpenAdapter)
  (THIS_ __in REFIID iid, UINT64 adapterId, __deref_out VOID * *ppAdapter) PURE;
};
...
inline constexpr IID IID_INDOverlapped = {
    ...
    0x75e2,
    0x4fe3,
    {0x8d, 0x6d, 0xd, 0xff, 0x36, 0xf0, 0x24, 0x42}};
...
DECLARE_INTERFACE_(INDOverlapped, IUnknown) {
  ...
  STDMETHOD(GetOverlappedResult)
  (THIS_ __inout OVERLAPPED * pOverlapped,
   __out SIZE_T * pNumberOfBytesTransferred, BOOL bWait) PURE;
};
...
DECLARE_INTERFACE_(INDCompletionQueue, INDOverlapped) {
  ...
  STDMETHOD(Resize)(THIS_ SIZE_T nEntries) PURE;

  STDMETHOD(Notify)(THIS_ DWORD Type, __inout OVERLAPPED * pOverlapped) PURE;

  STDMETHOD_(SIZE_T, GetResults)
  (THIS_ __out_ecount_part_opt(nResults, return) ND_RESULT * pResults[],
   SIZE_T nResults) PURE;
};
...
inline constexpr IID IID_INDEndpoint = {
    ...
    0xb679,
    0x44a9,
    {0xbd, 0x65, 0xe8, 0x2f, 0x3d, 0xe1, 0x2d, 0x1a}};
...
DECLARE_INTERFACE_(INDEndpoint, IUnknown) {
  ...
  STDMETHOD_(void, StartRequestBatch)(THIS) PURE;

  STDMETHOD_(void, SubmitRequestBatch)(THIS) PURE;

  STDMETHOD(Send)
  (THIS_ __out ND_RESULT * pResult, __in_ecount_opt(nSge) const ND_SGE* pSgl,
   SIZE_T nSge, DWORD Flags) PURE;

  STDMETHOD(SendAndInvalidate)
  (THIS_ __out ND_RESULT * pResult, __in_ecount_opt(nSge) const ND_SGE* pSgl,
   SIZE_T nSge, __in const ND_MW_DESCRIPTOR* pRemoteMwDescriptor, DWORD Flags)
      PURE;
  ...
  STDMETHOD(Bind)
  (THIS_ __out ND_RESULT * pResult, __in ND_MR_HANDLE hMr,
   __in INDMemoryWindow * pMw, __in_bcount(BufferSize) const void* pBuffer,
   SIZE_T BufferSize, DWORD Flags, __out ND_MW_DESCRIPTOR* pMwDescriptor) PURE;

  STDMETHOD(Invalidate)
  (THIS_ __out ND_RESULT * pResult, __in INDMemoryWindow * pMw, DWORD Flags)
      PURE;

  STDMETHOD(Read)
  (THIS_ __out ND_RESULT * pResult, __in_ecount_opt(nSge) const ND_SGE* pSgl,
   SIZE_T nSge, __in const ND_MW_DESCRIPTOR* pRemoteMwDescriptor,
   ULONGLONG Offset, DWORD Flags) PURE;
...
inline constexpr IID IID_INDConnector = {
    ...
    0xe274,
    0x4aeb,
    {0xac, 0x57, 0xcd, 0x5d, 0x43, 0x76, 0xe0, 0xb7}};
...
DECLARE_INTERFACE_(INDConnector, INDOverlapped) {
  ...
  (THIS_ __in INDCompletionQueue * pInboundCq,
   __in INDCompletionQueue * pOutboundCq, SIZE_T nInboundEntries,
   SIZE_T nOutboundEntries, SIZE_T nInboundSge, SIZE_T nOutboundSge,
   SIZE_T InboundReadLimit, SIZE_T OutboundReadLimit,
   __out_opt SIZE_T * pMaxInlineData, __deref_out INDEndpoint * *ppEndpoint)
      PURE;

  STDMETHOD(Connect)
  (THIS_ __in INDEndpoint * pEndpoint,
   __in_bcount(AddressLength) const struct sockaddr* pAddress,
   SIZE_T AddressLength, INT Protocol, USHORT LocalPort,
   __in_bcount_opt(PrivateDataLength) const void* pPrivateData,
   SIZE_T PrivateDataLength, __inout OVERLAPPED* pOverlapped) PURE;
  ...
  STDMETHOD(Reject)
  (THIS_ __in_bcount_opt(PrivateDataLength) const void* pPrivateData,
   SIZE_T PrivateDataLength) PURE;

  STDMETHOD(GetConnectionData)
  (THIS_ __out_opt SIZE_T * pInboundReadLimit,
   __out_opt SIZE_T * pOutboundReadLimit,
   __out_bcount_part_opt(*pPrivateDataLength,
                         *pPrivateDataLength) void* pPrivateData,
   __inout_opt SIZE_T* pPrivateDataLength) PURE;

  STDMETHOD(GetLocalAddress)
  (THIS_ __out_bcount_part_opt(*pAddressLength,
                               *pAddressLength) struct sockaddr *
       pAddress,
   __inout SIZE_T * pAddressLength) PURE;
...
inline constexpr IID IID_INDListen = {
    ...
    0xba3f,
    0x4441,
    {0x9f, 0xe1, 0x3b, 0x67, 0x95, 0xe4, 0xe6, 0x68}};
...
DECLARE_INTERFACE_(INDListen, INDOverlapped) {
  ...
  // *** INDListen methods ***
  STDMETHOD(GetConnectionRequest)
  (THIS_ __inout INDConnector * pConnector, __inout OVERLAPPED * pOverlapped)
      PURE;
};
...
DECLARE_INTERFACE_(INDAdapter, INDOverlapped) {
  ...
  // *** INDAdapter methods ***
  STDMETHOD_(HANDLE, GetFileHandle)(THIS) PURE;

  STDMETHOD(Query)
  (THIS_ DWORD VersionRequested,
   __out_bcount_part_opt(*pBufferSize, *pBufferSize) ND_ADAPTER_INFO * pInfo,
   __inout_opt SIZE_T * pBufferSize) PURE;

  STDMETHOD(Control)
  (THIS_ DWORD IoControlCode,
   __in_bcount_opt(InBufferSize) const void* pInBuffer, SIZE_T InBufferSize,
   __out_bcount_opt(OutBufferSize) void* pOutBuffer, SIZE_T OutBufferSize,
   __out SIZE_T* pBytesReturned, __inout OVERLAPPED* pOverlapped) PURE;

  STDMETHOD(CreateCompletionQueue)
  (THIS_ SIZE_T nEntries, __deref_out INDCompletionQueue * *ppCq) PURE;

  STDMETHOD(RegisterMemory)
  (THIS_ __in_bcount(BufferSize) const void* pBuffer, SIZE_T BufferSize,
   __inout OVERLAPPED* pOverlapped, __deref_out ND_MR_HANDLE* phMr) PURE;

  STDMETHOD(DeregisterMemory)
  (THIS_ __in ND_MR_HANDLE hMr, __inout OVERLAPPED * pOverlapped) PURE;

  STDMETHOD(CreateMemoryWindow)
  (THIS_ __out ND_RESULT * pInvalidateResult,
   __deref_out INDMemoryWindow * *ppMw) PURE;

  STDMETHOD(CreateConnector)
  (THIS_ __deref_out INDConnector * *ppConnector) PURE;

  STDMETHOD(Listen)
  (THIS_ SIZE_T Backlog, INT Protocol, USHORT Port,
   __out_opt USHORT * pAssignedPort, __deref_out INDListen * *ppListen) PURE;
};
...
inline constexpr IID IID_INDProvider = {
    ...
    0x5fdf,
    0x47e6,
    {0xb2, 0xd0, 0x2a, 0x6e, 0xda, 0x8d, 0x39, 0xdd}};
...
DECLARE_INTERFACE_(INDProvider, IUnknown) {
  ...
  // *** INDProvider methods ***
  STDMETHOD(QueryAddressList)
  (THIS_ __out_bcount_part_opt(*pBufferSize, *pBufferSize) SOCKET_ADDRESS_LIST *
       pAddressList,
   __inout SIZE_T * pBufferSize) PURE;

  STDMETHOD(OpenAdapter)
  (THIS_ __in_bcount(AddressLength) const struct sockaddr* pAddress,
   SIZE_T AddressLength, __deref_out INDAdapter** ppAdapter) PURE;
};
...
//          If this assert fires, it will raise a fatal error and crash the host
//          process. If AeDebug key is set, this will cause the debugger to
//          launch When debugger attached, the 'ahi' command can be used to
//          ignore the assertion and continue.
//        NOTE: Unlike the CRT assert, the strings for these asserts are stored
//        in the symbols,
//              so asserts do not add strings to the image.
//
//   InvariantAssertP/AssertP(exp_) :
//      This is the same as the InvariantAssert/Assert macro, except that it
//      will only passively fire
...
#define InvariantAssert(exp_)                                              \
  ((!(exp_))                                                               \
       ? (__annotation(L"Debug", L"AssertFail", L#exp_), __int2c(), FALSE) \
       : TRUE)

#define InvariantAssertP(exp_)                                             \
  ((!(exp_) && IsDebuggerPresent())                                        \
       ? (__annotation(L"Debug", L"PassiveAssertFail", L#exp_), __int2c(), \
          FALSE)                                                           \
       : TRUE)

#if DBG
#define Assert(exp_)       \
  __analysis_assume(exp_); \
  InvariantAssert(exp_)
#define AssertP(exp_)      \
  __analysis_assume(exp_); \
  InvariantAssertP(exp_)
#else
...
#define ASSERT_BENIGN(exp)                                           \
  (void)(!(exp) ? OutputDebugStringA("Assertion Failed:" #exp "\n"), \
         FALSE  : TRUE)
#else
#define ASSERT_BENIGN(exp)
#endif  // DBG
...
class Lock {
 ...
 public:
  ...
  _Releases_lock_(*this->m_pLock) ~Lock() { LeaveCriticalSection(m_pLock); }
};
//---------------------------------------------------------
...
template <class T, int Offset>
inline void List<T, Offset>::InsertBefore(LIST_ENTRY* pNext,
                                          LIST_ENTRY* pEntry) {
  pEntry->Flink = pNext;
...
template <class T, int Offset>
inline void List<T, Offset>::InsertAfter(LIST_ENTRY* pPrev,
                                         LIST_ENTRY* pEntry) {
  pEntry->Blink = pPrev;
...
#pragma prefast(disable : 28194, \
                "Linking the item into the list aliases the memory.");
template <class T, int Offset>
...
template <class T, int Offset>
inline typename List<T, Offset>::iterator List<T, Offset>::insert(iterator it,
                                                                  T& item) {
  LIST_ENTRY* pEntry = item2entry(item);
...
class Provider {
 ...
 public:
  ...
  virtual HRESULT OpenAdapter(_In_ REFIID iid,
                              _In_bytecount_(cbAddress)
                                  const struct sockaddr* pAddress,
                              _In_ ULONG cbAddress,
                              _Deref_out_ VOID** ppIAdapter) PURE;

  virtual HRESULT QueryAddressList(
      _Out_opt_bytecap_post_bytecount_(*pcbAddressList, *pcbAddressList)
          SOCKET_ADDRESS_LIST* pAddressList,
      _Inout_ ULONG* pcbAddressList) PURE;
...
class NdV1Provider : public Provider {
 public:
  ...
  HRESULT OpenAdapter(_In_ REFIID iid,
                      _In_bytecount_(cbAddress) const struct sockaddr* pAddress,
                      _In_ ULONG cbAddress,
                      _Deref_out_ VOID** ppIAdapter) override;

  HRESULT QueryAddressList(_Out_opt_bytecap_post_bytecount_(*pcbAddressList,
                                                            *pcbAddressList)
                               SOCKET_ADDRESS_LIST* pAddressList,
                           _Inout_ ULONG* pcbAddressList) override;
...
class Framework {
 ...
 public:
  ...
  HRESULT QueryAddressList(_In_ DWORD flags,
                           _Out_opt_bytecap_post_bytecount_(*pcbAddressList,
                                                            *pcbAddressList)
                               SOCKET_ADDRESS_LIST* pAddressList,
                           _Inout_ SIZE_T* pcbAddressList);
  ...
  HRESULT CheckAddress(_In_bytecount_(cbAddress)
                           const struct sockaddr* pAddress,
                       _In_ SIZE_T cbAddress);

  HRESULT OpenAdapter(_In_ REFIID iid,
                      _In_bytecount_(cbAddress) const struct sockaddr* pAddress,
                      _In_ SIZE_T cbAddress, _Deref_out_ VOID** ppIAdapter);

  void FlushProvidersForUser();

 private:
  static HRESULT ValidateAddress(_In_bytecount_(cbAddress)
                                     const struct sockaddr* pAddress,
                                 _In_ SIZE_T cbAddress);

  static void CountAddresses(_In_ const List<Address>& list,
                             _Inout_ SIZE_T* pnV4, _Inout_ SIZE_T* pnV6);

  static void BuildAddressList(_In_ Provider& prov,
                               _In_ const SOCKET_ADDRESS_LIST& addrList,
                               _Inout_ List<Address>* pList);

  static void CopyAddressList(_In_ const List<Address>& list,
                              _Inout_ SOCKET_ADDRESS_LIST* pAddressList,
                              _Inout_ BYTE** ppBuf, _Inout_ SIZE_T* pcbBuf);
...
inline Address::Address(_In_ const struct sockaddr& addr,
...
inline bool Address::Matches(const struct sockaddr* pAddr) const {
  switch (pAddr->sa_family) {
    case AF_INET:
      return reinterpret_cast<const struct sockaddr_in*>(pAddr)
                 ->sin_addr.S_un.S_addr == m_Addr.Ipv4.sin_addr.S_un.S_addr;

    case AF_INET6:
      return (
          ::memcmp(
              reinterpret_cast<const sockaddr_in6*>(pAddr)->sin6_addr.u.Byte,
              m_Addr.Ipv6.sin6_addr.u.Byte,
              sizeof(m_Addr.Ipv6.sin6_addr)) == 0);
...
inline SIZE_T Address::CopySockaddr(_Out_writes_(len) BYTE* pBuf,
                                    _In_ SIZE_T len) const {
...
inline HRESULT Provider::Init(GUID& ProviderGuid) {
  ...
  pPath = static_cast<WCHAR*>(::HeapAlloc(ghHeap, 0, sizeof(WCHAR) * MAX_PATH));
  if (pPath == nullptr) {
    return ND_NO_MEMORY;
  }
  ...
  m_Path = static_cast<WCHAR*>(::HeapAlloc(ghHeap, 0, sizeof(WCHAR) * pathLen));
  if (m_Path == nullptr) {
    ::HeapFree(ghHeap, 0, pPath);
    return ND_NO_MEMORY;
  }
...
//
inline HRESULT Provider::GetClassObject(_In_ const IID& iid,
  ...
  if (m_hProvider == nullptr) {
    ...
    HMODULE hCurrentProvider =
        static_cast<HMODULE>(::InterlockedCompareExchangePointer(
  ...
  HRESULT hr = m_pfnDllGetClassObject(m_Guid, iid, ppInterface);
...
inline NdV1Provider::NdV1Provider() : Provider(ND_VERSION_1) {}
...
inline HRESULT NdV1Provider::GetProvider(INDProvider** ppIProvider) {
  ...
  HRESULT hr = GetClassObject(detail::iid_i_class_factory,
                              reinterpret_cast<void**>(&pClassFactory));
  if (FAILED(hr)) {
    return hr;
  }

  hr = pClassFactory->CreateInstance(nullptr, IID_INDProvider,
...
inline HRESULT NdV1Provider::OpenAdapter(_In_ REFIID iid,
                                         _In_bytecount_(cbAddress)
                                             const struct sockaddr* pAddress,
                                         _In_ ULONG cbAddress,
                                         _Deref_out_ VOID** ppIAdapter) {
  ...
  hr = pIProvider->OpenAdapter(pAddress, cbAddress,
...
inline HRESULT NdV1Provider::QueryAddressList(
    _Out_opt_bytecap_post_bytecount_(*pcbAddressList, *pcbAddressList)
        SOCKET_ADDRESS_LIST* pAddressList,
...
inline NdProvider::NdProvider() : Provider(ND_VERSION_2) {}
...
// callers may call this function concurrently.
//
inline HRESULT NdProvider::OpenAdapter(_In_ REFIID iid,
                                       _In_bytecount_(cbAddress)
                                           const struct sockaddr* pAddress,
                                       _In_ ULONG cbAddress,
                                       _Deref_out_ VOID** ppIAdapter) {
  ...
  HRESULT hr =
      GetClassObject(IID_IND2Provider, reinterpret_cast<void**>(&pIProvider));
  if (FAILED(hr)) {
    TryUnload();
    return ND_INVALID_ADDRESS;
  }
...
inline HRESULT NdProvider::QueryAddressList(
    _Out_opt_bytecap_post_bytecount_(*pcbAddressList, *pcbAddressList)
        SOCKET_ADDRESS_LIST* pAddressList,
    _Inout_ ULONG* pcbAddressList) {
  IND2Provider* pIProvider;
...
//
inline constexpr int ND_SERVICE_FLAGS1 =
    (XP1_GUARANTEED_DELIVERY | XP1_GUARANTEED_ORDER | XP1_MESSAGE_ORIENTED |
     XP1_CONNECT_DATA);
inline constexpr int ND_PROVIDER_FLAGS =
    (PFL_HIDDEN | PFL_NETWORKDIRECT_PROVIDER);
...
inline HRESULT Framework::Init() {
  ...
  HANDLE hIocp = ::CreateIoCompletionPort(reinterpret_cast<HANDLE>(m_Socket),
                                          m_hIocp, ND_NOTIFY_ADDR_CHANGE, 0);
  if (hIocp != m_hIocp) {
    return HRESULT_FROM_WIN32(::GetLastError());
  }
  ...
  hIocp = ::CreateIoCompletionPort(m_hProviderChange, m_hIocp,
                                   ND_NOTIFY_PROVIDER_CHANGE, 0);
  if (hIocp != m_hIocp) {
    return HRESULT_FROM_WIN32(::GetLastError());
  }
  ...
  DWORD BytesRet;
  ret = ::WSAIoctl(m_Socket, SIO_ADDRESS_LIST_CHANGE, nullptr, 0, nullptr, 0,
                   &BytesRet, &m_Ov[ND_NOTIFY_ADDR_CHANGE], nullptr);
...
inline ULONG Framework::AddRef() { return ::InterlockedIncrement(&m_nRef); }

inline ULONG Framework::Release() {
  ASSERT(m_nRef > 0);
...
inline HRESULT Framework::QueryAddressList(
    _In_ DWORD flags,
    _Out_opt_bytecap_post_bytecount_(*pcbAddressList, *pcbAddressList)
        SOCKET_ADDRESS_LIST* pAddressList,
    _Inout_ SIZE_T* pcbAddressList) {
  ...
  SIZE_T cbRequired = sizeof(SOCKET_ADDRESS_LIST) - sizeof(SOCKET_ADDRESS) +
                      ...
                      (sizeof(sockaddr_in) * nV4) +
                      (sizeof(sockaddr_in6) * nV6);
  ...
  BYTE* pBuf = reinterpret_cast<BYTE*>(&pAddressList->Address[(nV4 + nV6)]);
  SIZE_T cbRemaining =
      *pcbAddressList - FIELD_OFFSET(SOCKET_ADDRESS_LIST, Address[(nV4 + nV6)]);
...
inline HRESULT Framework::ResolveAddress(
    _In_bytecount_(cbRemoteAddress) const struct sockaddr* pRemoteAddress,
    _In_ SIZE_T cbRemoteAddress,
    _Out_bytecap_(*pcbLocalAddress) struct sockaddr* pLocalAddress,
    _Inout_ SIZE_T* pcbLocalAddress) {
  ...
  for (List<Address>::iterator pAddr = m_NdV1AddrList.begin();
       pAddr != m_NdV1AddrList.end(); ++pAddr) {
    if (pAddr->Matches(pLocalAddress)) {
      return S_OK;
    }
  }
...
inline HRESULT Framework::CheckAddress(_In_bytecount_(cbAddress)
                                           const struct sockaddr* pAddress,
                                       _In_ SIZE_T cbAddress) {
  ASSERT(pAddress);
...
inline HRESULT Framework::OpenAdapter(_In_ REFIID iid,
                                      _In_bytecount_(cbAddress)
                                          const struct sockaddr* pAddress,
                                      _In_ SIZE_T cbAddress,
                                      _Deref_out_ VOID** ppIAdapter) {
  ...
  for (List<Address>::iterator pAddr = pList->begin(); pAddr != pList->end();
       ++pAddr) {
    if (!pAddr->Matches(pAddress)) {
      continue;
    }
    ...
    hr = pAddr->GetProvider()->OpenAdapter(
        iid, pAddress, static_cast<ULONG>(cbAddress), ppIAdapter);
    if (FAILED(hr)) {
...
inline void Framework::FlushProvidersForUser() {
  Lock lock(&m_lock);
  FlushProviders();
}

inline HRESULT Framework::ValidateAddress(_In_bytecount_(cbAddress)
                                              const struct sockaddr* pAddress,
                                          _In_ SIZE_T cbAddress) {
...
inline void Framework::CountAddresses(_In_ const List<Address>& list,
                                      _Inout_ SIZE_T* pnV4,
                                      _Inout_ SIZE_T* pnV6) {
  for (List<Address>::iterator pAddr = list.begin();
       pAddr != list.end() && (*pnV4 + *pnV6) < INT_MAX; ++pAddr) {
    switch (pAddr->AF()) {
      case AF_INET:
        (*pnV4)++;
        break;
...
inline void Framework::BuildAddressList(
    _In_ Provider& prov, _In_ const SOCKET_ADDRESS_LIST& addrList,
    _Inout_ List<Address>* pList) {
  for (int i = 0; i < addrList.iAddressCount; i++) {
    ...
    switch (addrList.Address[i].lpSockaddr->sa_family) {
      case AF_INET:
        if (addrList.Address[i].iSockaddrLength < sizeof(struct sockaddr_in)) {
          continue;
        }
        break;

      case AF_INET6:
        if (addrList.Address[i].iSockaddrLength < sizeof(struct sockaddr_in6)) {
          continue;
        }
        break;
    ...
    Address* pAddr = new Address(*addrList.Address[i].lpSockaddr, prov);
    if (pAddr != nullptr) {
...
inline void Framework::CopyAddressList(
    _In_ const List<Address>& list, _Inout_ SOCKET_ADDRESS_LIST* pAddressList,
    _Inout_ BYTE** ppBuf, _Inout_ SIZE_T* pcbBuf) {
  INT nAddress = pAddressList->iAddressCount;

  for (List<Address>::iterator pAddr = list.begin();
       pAddr != list.end() && nAddress < INT_MAX; ++pAddr) {
    ...
    ASSERT(len < INT_MAX);
    pAddressList->Address[nAddress].iSockaddrLength = static_cast<INT>(len);
...
inline void Framework::ProcessUpdates() {
  BOOL ret;
    ...
    if (ret == TRUE) {
      ...
      switch (key) {
        ...
        case ND_NOTIFY_ADDR_CHANGE:
          ...
          status =
              ::WSAIoctl(m_Socket, SIO_ADDRESS_LIST_CHANGE, nullptr, 0, nullptr,
...
inline void Framework::ProcessProviderChange() {
  ...
  WSAPROTOCOL_INFOW* pProtocols =
      static_cast<WSAPROTOCOL_INFOW*>(::HeapAlloc(ghHeap, 0, len));
  ...
  for (List<Provider>::iterator pProv = m_ProviderList.begin();
       pProv != m_ProviderList.end(); ++pProv) {
    pProv->MarkInactive();
  }
...
inline void Framework::ProcessAddressChange() {
...
inline void Framework::FlushProviders() {
...
EXTERN_C inline HRESULT ND_HELPER_API NdStartup(VOID) {
  LONG init;
  do {
...
EXTERN_C inline HRESULT ND_HELPER_API NdCleanup(VOID) {
  LONG init;
  do {
    init = ::InterlockedCompareExchange(&NetworkDirect::gInitializing, 1, 0);
  } while (init == 1);
...
EXTERN_C inline VOID ND_HELPER_API NdFlushProviders(VOID) {
  if (NetworkDirect::gpFramework == nullptr) {
    return;
  }
...
// Network capabilities
//
EXTERN_C inline HRESULT ND_HELPER_API NdQueryAddressList(
    _In_ DWORD flags,
    _Out_opt_bytecap_post_bytecount_(*pcbAddressList, *pcbAddressList)
        SOCKET_ADDRESS_LIST* pAddressList,
    _Inout_ SIZE_T* pcbAddressList) {
  ...
  return NetworkDirect::gpFramework->QueryAddressList(flags, pAddressList,
                                                      pcbAddressList);
}

EXTERN_C inline HRESULT ND_HELPER_API NdResolveAddress(
    _In_bytecount_(cbRemoteAddress) const struct sockaddr* pRemoteAddress,
    _In_ SIZE_T cbRemoteAddress,
    _Out_bytecap_(*pcbLocalAddress) struct sockaddr* pLocalAddress,
    _Inout_ SIZE_T* pcbLocalAddress) {
  ...
  return NetworkDirect::gpFramework->ResolveAddress(
      pRemoteAddress, cbRemoteAddress, pLocalAddress, pcbLocalAddress);
}

EXTERN_C inline HRESULT ND_HELPER_API
NdCheckAddress(_In_bytecount_(cbAddress) const struct sockaddr* pAddress,
               _In_ SIZE_T cbAddress) {
  if (NetworkDirect::gpFramework == nullptr) {
    return ND_DEVICE_NOT_READY;
  }
...
EXTERN_C inline HRESULT ND_HELPER_API NdOpenAdapter(
    _In_ REFIID iid, _In_bytecount_(cbAddress) const struct sockaddr* pAddress,
    _In_ SIZE_T cbAddress, _Deref_out_ VOID** ppIAdapter) {
  if (NetworkDirect::gpFramework == nullptr) {
    return ND_DEVICE_NOT_READY;
  }

  return NetworkDirect::gpFramework->OpenAdapter(iid, pAddress, cbAddress,
                                                 ppIAdapter);
}

EXTERN_C inline HRESULT ND_HELPER_API
NdOpenV1Adapter(_In_bytecount_(cbAddress) const struct sockaddr* pAddress,
                _In_ SIZE_T cbAddress, _Deref_out_ INDAdapter** ppIAdapter) {
  if (NetworkDirect::gpFramework == nullptr) {
    return ND_DEVICE_NOT_READY;
  }

  return NetworkDirect::gpFramework->OpenAdapter(
      IID_INDAdapter, pAddress, cbAddress,
