...
class nd_connect_op_base : public nd_op_base {
 ...
 protected:
  ...
  inline status_t process_complete_connect(void* owner, asio::error_code& ec);
...
inline nd_connect_op_base::status_t
nd_connect_op_base::process_complete_connect(void* owner,
                                             asio::error_code& ec) {
  ...
  if (FAILED(hr)) {
    ...
    if (state_)
      state_->store(connect_state::closed, std::memory_order_release);
...
inline bool nd_connect_op_base::mark_connected(asio::error_code& ec) {
  if (state_) {
    ...
    if (!state_->compare_exchange_strong(expected, connect_state::connected,
...
inline void nd_connect_op_base::capture_remote_pd() {
  ...
  if (SUCCEEDED(hr) || hr == ND_BUFFER_OVERFLOW) {
    reply_len_ = (std::min)(static_cast<std::size_t>(pd_size), reply_.size());
  }
}
