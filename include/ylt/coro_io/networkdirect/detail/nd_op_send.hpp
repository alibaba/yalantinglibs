...
template <mr_const_buffer_sequence BufferSequence, typename Handler,
          typename IoExecutor>
class nd_send_op final : public nd_two_sided_op<BufferSequence> {
 public:
 ...
 public:
  ASIO_DEFINE_HANDLER_PTR(nd_send_op);
  nd_send_op(asio::error_code const& ec, BufferSequence const& buffer_sequence,
             Handler& handler, IoExecutor const& io_ex)
      : base_type(&nd_send_op::do_complete, ec, buffer_sequence),
        handler_(ASIO_MOVE_CAST(Handler)(handler)),
        work_(handler_, io_ex) {}

  op_type get_op_type() const noexcept { return op_type::post_send; }

 private:
  static void do_complete(void* owner, nd_verbs_op_base* base,
                          asio::error_code const& ec,
                          std::size_t /*bytes_transferred*/) {
    nd_send_op* o = static_cast<nd_send_op*>(base);
    if (!ec) {
