...
summary内部采用浮点数分桶的策略进行统计。内部的桶被分为128段，只有当插入的数据命中了该段桶时才会动态分配内存。summary的内存上限为130KB，不过通常情况下，输入数据只会命中部分的分段，此时的内存占用会小得多。经测试，一个具有1万个label，每个label随机写入10万个1-1048576的随机数的summary，占用内存约为64MB，单个summary占用6.4KB。
