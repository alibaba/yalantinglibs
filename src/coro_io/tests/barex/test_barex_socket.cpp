...
async_simple::coro::Lazy<std::error_code> echo_connect(
    std::vector<std::function<
        async_simple::coro::Lazy<std::error_code>(coro_io::barex_socket_t&)>>
        functions) {
  coro_io::barex_socket_t soc = std::make_shared<coro_io::barex_socket_impl_t>(
      coro_io::barex_socket_t::config_t{.buffer_size = 8 * 1024});
...
async_simple::coro::Lazy<std::error_code> test_read(
    coro_io::barex_socket_t& soc, std::size_t data_size,
...
async_simple::coro::Lazy<std::error_code> test_read_some(
  ...
  if (!ec) {
    auto sz =
        std::min<std::size_t>(data_size, soc.get_impl()->get_buffer_size());
    if (got_size != 0) {
      sz = got_size;
    }
    CHECK(len == sz);
...
async_simple::coro::Lazy<std::error_code> test_write(
  ...
  if (never_run) {
    auto result = co_await coro_io::sleep_for(std::chrono::seconds{100000},
                                              soc.get_coro_executor());
...
async_simple::coro::Lazy<std::error_code> write_iov(
    coro_io::barex_socket_t& soc, std::size_t data_size, std::size_t iov_size) {
...
async_simple::coro::Lazy<std::error_code> read_iov(coro_io::barex_socket_t& soc,
...
async_simple::coro::Lazy<std::error_code> rpc_like_send(
    coro_io::barex_socket_t& soc, std::size_t body_sz) {
  std::string body;
...
async_simple::coro::Lazy<std::error_code> sleep(coro_io::barex_socket_t& soc,
...
TEST_CASE("test socket io") {
  ...
  }
  {
    ELOG_WARN << "test read/write fix size, very bigger than rdma "
                 "buffer";
    auto result = async_simple::coro::syncAwait(
        collectAll(echo_accept({test(test_read, 350 * 1024)}),
                   echo_connect({test(test_write, 350 * 1024)})));
    auto& ec1 = std::get<0>(result);
    auto& ec2 = std::get<1>(result);
    CHECK_MESSAGE(!ec1.value(), ec1.value().message());
    CHECK_MESSAGE(!ec2.value(), ec2.value().message());
