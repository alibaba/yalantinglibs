...
TEST_CASE("test SmoothBurstyRateLimiter multi coroutine") {
  ...
  double expected_cost = (num_of_coroutine * permits_to_acquire_every_coroutine - 1) * (1 / permits_per_second);
  double cost_diff = 0.1;
  ...
  syncAwait(consumerListLazy().via(
    coro_io::g_block_io_context_pool<coro_io::multithread_context_pool>(1).get_executor()));
  double cost = (currentTimeMills() - start_mills) / 1000.0;
