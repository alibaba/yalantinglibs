#pragma once
#include <ylt/struct_pb0.hpp>

#include "ScopedTimer.hpp"
#include "no_op.h"
#include "sample.hpp"

namespace pb_sample0 {
...
struct struct_pb_sample0 : public base_sample {
  static inline constexpr LibType lib_type = LibType::STRUCT_PB0;
  std::string name() const override { return get_lib_name(lib_type); }

  void create_samples() override {
    rects_.list = {pb_sample0::create_rects(OBJECT_COUNT)};
    persons_.list = {pb_sample0::create_persons(OBJECT_COUNT)};
    monsters_.list = {pb_sample0::create_monsters(OBJECT_COUNT)};
  }

  void do_serialization() override {
    serialize(SampleType::RECT, rects_.list[0]);
    struct_pb0::to_pb(rects_, buffer_);
    struct_pb0::to_pb(rects_, buffer_);
    struct_pb0::to_pb(rects_, buffer_);
 ...
 private:
  ...
  pb_sample0::rects rects_;
  pb_sample0::persons persons_;
  pb_sample0::Monsters monsters_;
  std::string buffer_;
};
